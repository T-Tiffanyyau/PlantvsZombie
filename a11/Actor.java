 ...
 * @author dejohnso
 * 
 *         Notes: main properties of an actor are that it can - Attack (defend).
 * 
 */
public abstract class Actor extends Sprite {
	 ...
	 * @param speed        the number of pixels this moves per turn, positive values
	 *                     move left
	 * @param attackDamage the amount of health this deducts from enemies per attack
	 */
	public Actor(int xPosition, int yPosition, int size, String imgPath, int health, int coolDown, int speed,
			int attackDamage) {
	...
	/**
	 * Move this object horizontally. This method should be called in turns where
	 * this actor is allowed to move, i.e. it should not be colliding with something
	 * else.
	 */
	public void move() {
	...
	/**
	 * Update the internal state of the Actor each turn. This mainly involves
	 * decrementing the cooldown counter.
	 */
	public void update() {
		coolDownCounter--;
	}
	...
	/**
	 * Modify the attackDamage by change value.
	 * 
	 * @param change
	 */
	protected void changeAttackDamage(int change) {
		attackDamage += change;
	}
	 ...
	 * 
	 * This should only happen if: - Other is close enough to this actor. By default
	 * they must be touching. - This actor is ready to act again, based on its
	 * cooldown. - This actor can act on whatever type other has.
	 */
	...
	/**
	 * This method should be overidden in subclasses to specify plant-specific
	 * actions.
	 */
	protected abstract void actOn(Plant other);

	/**
	 * This method should be overidden in subclasses to specify zombie-specific
	 * actions.
	 */
